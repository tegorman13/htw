...
pd_train <- nestSbjModelFit |> mutate(pp=furrr::future_pmap(list(id,Model,Fit_Method,data), ~{
   generate_data(..2, ..4 |> mutate(id=..1), ds, num_samples = 10, return_dat="train_data")
   })) |>
  select(Fit_Method,pp,-data) |>
...
full_comparison |> round_tibble(1) |> select(-Error_difference,-nGrp) |> reactable(filterable=T,defaultPageSize=15)
