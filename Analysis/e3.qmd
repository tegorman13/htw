...
# testE3 |>
#    select(id,condit,bandOrder,vb,band,lowBound,highBound,vx) |>
#   group_by(id,condit,bandOrder,vb,highBound) |> 
#   summarize(vx=mean(vx)) |> 
#   group_by(condit,bandOrder,vb,highBound) |>
#   summarise(mvx=mean(vx),sdvx=sd(vx), n=n(),se= sdvx/sqrt(n),.groups="drop" ) |>
#   mutate(vbn=highBound-200, rectWidth=.2,
#           band_index=as.numeric(factor(vb)),
#           rectX = as.numeric(condit) ) |>
#   ggplot(aes(x=condit,y=mvx,group=vb)) +
#   geom_bar(stat="identity",aes(fill=vb),position="dodge") +
#   geom_errorbar(aes(ymin=mvx-se,ymax=mvx+se,group=vb),position="dodge",width=.9,alpha=.5) +
#   geom_segment(aes(x=band_index-.25,xend=rectX+.05,y=highBound,yend=highBound, col=vb,group=vb),
#                alpha=.8,linetype="dashed",position = "dodge") +
#   # geom_hline(data=vbRect,aes(yintercept=vbn,col=vb),alpha=.5,linetype="dashed") +
#   # geom_hline(data=vbRect,aes(yintercept=highBound,col=vb),alpha=.5,linetype="dashed") +
#   scale_x_discrete(labels = c("Constant", "Varied")) +
#   scale_y_continuous(expand=expansion(add=100),breaks=round(seq(0,2000,by=200),2)) +
#   facet_wrap(~bandOrder,ncol=1)
  # geom_rect(aes(xmin = rectX-.3, xmax = rectX+.3,
  #               ymin = vbn, ymax = highBound),
...
plot_data |> 
  ggplot(aes(x=bandInt,y=pred_y,color=interaction(condit, bandOrder))) +
  geom_point() +
  geom_abline(
    mapping = aes(
      intercept = intercept,
