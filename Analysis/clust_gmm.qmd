...
#| fig.width=12
#| 
ta <- test %>% group_by(id,condit, vb) %>% summarise(mean(dist),mean(vx),mad(vx),sd(vx),IQR(vx),moments::skewness(vx))

plots <- map(names(ta)[-(1:3)], ~ {
  var_name <- .x
  ggplot(ta, aes(x = vb, y = !!sym(var_name), color = condit)) +
    gghalves::geom_half_violin()+
    gghalves::geom_half_boxplot(alpha=.5)+
    labs(x = "Velocity Band", y = var_name,
         title = paste("Plot of", var_name, "by Velocity Band and Condition"),
         color = "Condition") +
    theme_minimal()
})
...
dispersion_vars <- names(ta)[-(1:5)]
correlation_results <- map(dispersion_vars, ~ {
  var_name <- .x
  cor_result <- cor.test(ta[['mean(dist)']], ta[[var_name]], method = "pearson")
  list(var_name = var_name, cor_result = cor_result)
})
...
dispersion_vars <- names(ta)[-(1:5)]
dispersion_vars <- c('mean(dist)', dispersion_vars)
ta_subset <- ta[dispersion_vars]
correlation_matrix <- cor(ta_subset, method = "pearson")
correlation_matrix

library(corrplot)

correlation_matrices <- ta %>% 
  group_by(condit) %>% 
  nest() %>% 
  mutate(correlation_matrix = map(data, ~ cor(.x[dispersion_vars], method = "pearson")))

# Create a correlation plot for each condition
correlation_plots <- correlation_matrices %>% 
  mutate(plot = map(correlation_matrix, ~ corrplot(.x, method = "color")))

# Print the correlation plots
correlation_plots$plot


GGally::ggcorr(ta[-2], aes(colour = condit), method = c("everything", "pearson"))
...
x <- list(test$vx,test$vy)
out <- init.EM(x, nclass = 1)
...
fitK <- kmeans(test$vx,6)

ggplot(test,aes(x=vx,group=as.factor(fitK$cluster),fill=as.factor(fitK$cluster)))+geom_histogram()
ggplot(test,aes(x=vx,group=as.factor(fitK$cluster),fill=as.factor(fitK$cluster)))+geom_density()
...
for (i in 1:n_clusters) {
  ...
  km.out <- kmeans(test$vx, centers = i, nstart = 20)
...
test %>% group_by(vb) %>% summarise(mean(vx),mad(vx),sd(vx),IQR(vx))
...
NbClust(testS |> select(vx),distance="euclidean",min.nc=2,max.nc=6,method="average")
