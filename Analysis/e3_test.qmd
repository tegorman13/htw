...
In experiment 3, the overall effect of training condition shows a similar pattern to that of Experiment 2, with a  moderately strong effect tendency for the varied group to show lower deviation (β = `r cd1$estimate`, 95% CrI \[`r cd1$conf.low`, `r cd1$conf.high`\]), with 97% of the posterior distribution falling under 0. (NEED TO CONTROL FOR BAND ORDER HERE)
...
testE3 |> group_by(id,vb,condit,bandOrder) |> plot_distByCondit() + facet_wrap(bandOrder~condit,scale="free_x")
...
See @tbl-e3-bmm-vx for the full model results. The estimated coefficient for training condition (β = `r cd1$estimate`, 95% CrI \[`r cd1$conf.low`, `r cd1$conf.high`\]) suggests that the varied group tends to produce softer throws than the constant group. More relevant to the issue of discrimination is the slope on Velocity Band (β = `r sc1$estimate`, 95% CrI \[`r sc1$conf.low`, `r sc1$conf.high`\]).
Interaction: (β = `r intCoef1$estimate`, 95% CrI \[`r intCoef1$conf.low`, `r intCoef1$conf.high`\])


It's also noteworthy that the difference between the E3 slope and E1 & E2 slope is quite strong, with the upper end of the 95% CI for E3 falling underneath the lower end of the 95% CI for both of the earlier experiments. 
