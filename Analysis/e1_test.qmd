...
See @tbl-e1-bmm-vx for the full model results. The estimated coefficient for training condition (β = 146, 95% CI [35.96, 254.70]) suggests that the varied group tends to produce harder throws than the constant group. Most relevant to the issue of discrimination is the slope on Velocity Band (β = 0.69, 95% CI [.59, .77]). Although the median slope does fall underneath the ideal of value of 1, the fact that the 95% credible interval does not contain 0 provides strong evidence that participants exhibited some discrimination between bands. The slope coefficient (β = 0.69 ) also seems in accordance with an examination of @tbl-e1-test-nf-vx, which shows that average x-velocity fell within the range of the target band for four out of six of the bands ($4/6$=.67). The estimate for the interaction between slope and condition (β = -.12, 95% CI [-.25, 0.00]), suggests that the varied condition had significantly lower slopes than did the constant group. This difference is depicted visually in @fig-e1-bmm-bx. 
...
condEffectsPlot <- ggplot(aes(x = bandInt, y = .value, color = condit, fill = condit)) + 
...
cdMed %>% 
  group_by(condit) %>%  
  reframe(enframe(quantile(Slope, c(0.0,0.25, 0.5, 0.75,1)), "quantile", "Slope")) |> 
...
# cdMed %>% ggplot(aes(x=Slope)) + 
#   stat_halfeye(alpha=.5) +facet_wrap(~condit)
...
combined_df |> indv_model_plot(indv_coefs, testAvg, rank_variable=Estimate.bandInt,n_sbj=4,"max")
combined_df |> indv_model_plot(indv_coefs, testAvg, rank_variable=Estimate.bandInt,n_sbj=4,"min")
