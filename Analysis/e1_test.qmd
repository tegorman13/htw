...
vx_pred  |>
  ...
  ggplot(aes(x=vb,y=vx_pred, group = vb,fill=vb)) + 
  ...
  geom_point(aes(x = as.numeric(vb) - 0.15, y = vx_pred, colour = vb),
             ...
             size = 1,
             shape = 20) +
  geom_boxplot(aes(x = vb, y = vx_pred, fill = vb),
               outlier.shape = NA,
               alpha = 0.5,
               width = 0.1,
               colour = "black") +
  geom_hline(yintercept = 0,
             ...
             color = 'red',
             size = 0.4) + 
  coord_flip() + facet_wrap(~condit,ncol=2) + ggtitle("Predicted Vx")  } /
{
vx_pred  |>
  filter(iter < 2)  |> group_by(id,condit,vb) |>
  summarise(vx=mean(vx)) %>%
  ggplot(aes(x=vb,y=vx, group = vb,fill=vb)) + 
  geom_flat_violin( position = position_nudge(x = 0.1, y = 0),
                   adjust = 1.5,
                   trim = FALSE,
                   alpha = .5,
                   colour = NA) +
  geom_point(aes(x = as.numeric(vb) - 0.15),
             # position = position_jitter(width = 0.05),
             position = position_jitter(width = 0.05, height = 0),
             size = 1,
             shape = 20) +
  ...
  geom_hline(yintercept = 0,
             ...
             color = 'red',
             size = 0.4) + 
  coord_flip() + facet_wrap(~condit,ncol=2) + ggtitle("Empirical Vx") }
```
