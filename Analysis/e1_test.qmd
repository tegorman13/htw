...
random_effects <- e1_vxBMM |> 
 ...
 indvDraws <- left_join(random_effects, fixed_effects, by = join_by(".chain", ".iteration", ".draw")) |> 
  ...
  indvSlopes |> reframe(enframe(quantile(SlopeInt, c(0.0,0.25, 0.5, 0.75,1)), "quantile", "SlopeInt")) |> 
...
#| fig-width: 10
  ...
  indvSlopes |> ggplot(aes(y=rankSlope, x=Slope,fill=condit,color=condit)) + 
...
#| fig-width: 12
nSbj <- 3
indvDraws  |> indv_model_plot(indv_coefs, testAvg, Slope,rank_variable=Estimate.bandInt,n_sbj=nSbj,"max")
indvDraws |> indv_model_plot(indv_coefs, testAvg,Slope, rank_variable=Estimate.bandInt,n_sbj=nSbj,"min")
