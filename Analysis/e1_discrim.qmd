...
test <- e1 |> filter(expMode2 == "Test") |> mutate(id=factor(id,levels=unique(id)))
...
# fit model
m1_bayes <- brm(censored_vx | cens(censored) ~ 1 + bandInt + condit + (1 + bandInt | id), 
                data = test, family = gaussian())

m1_bayesT <- brm(censored_vx | trunc(lb = 0) ~ 1 + bandInt + condit + (1 + bandInt | id), 
                data = test, family = lognormal(),chains=1)
