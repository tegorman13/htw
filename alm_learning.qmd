...
source("Functions/alm_functions.R")
...
pf <- function(df){
  lc = df %>% group_by(id,c,lr, update_func_name, simNum, trial, input,inNodes,outNodes,noise) %>%
  summarize(almTrainDat = mean(almTrainDat), .groups = "drop") %>%
    mutate(simLab=paste0("c:",c,",lr:",lr,",no=",noise,",in:",inNodes,",out:",outNodes,"um:",update_func_name)) %>%
...
#| warning: false
 tibble(crossing(
  ...
  unnest(c(almTrainDat, td)) %>% select(-d) %>% mutate(input=as.factor(input)) %>% pf(.)
