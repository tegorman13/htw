...
The Value column of the table reflects the minimum RMSE achieved when fitting the model. The Test_RMSE column is the main performance indicator of interest, and represents the RMSE for just the testing data - thus for the Test Only fits, the Value and Test_RMSE columns are identical - given that the models were trained and tested on the exact some data sets.  For the Test & Train, and Train Only fits - the Test_RMSE can diverge from the fitting values. 
