...
pmod2 <- e2_model$post_dat_l |> filter(!(Resp=="Observed")) |> 
  group_by(id,condit, Fit_Method,Resp,bandType,x,vb) |> 
 summarize(vx=median(val)) |> 
 ggplot( aes(x=condit,y=vx, fill=vb,col=ifelse(bandType=="Trained","black",NA),size=ifelse(bandType=="Trained","black",NA))) + 
  stat_bar + 
...
(pemp2)  / pmod2 + 
  plot_layout(design = layout, heights = unit(c(4, 9), c('cm','null')))
```
...
e3_model <- load_e3()
post_tabs3 <- abc_tables(e2_model$post_dat,e3_model$post_dat_l)
train_tab3 <- abc_train_tables(e2_model$pd_train,e3_model$pd_train_l)
